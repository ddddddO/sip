...
func isValidRequestACK(b []byte) bool {
	requestMsg := bytes.Split(b, end)
	// check request line
	if !isValidRequestLine(requestMsg[0], methodACK) {
		return false
	}
...
func isValidRequestBYE(b []byte) bool {
	requestMsg := bytes.Split(b, end)
	// check request line
	if !isValidRequestLine(requestMsg[0], methodBYE) {
		return false
	}
